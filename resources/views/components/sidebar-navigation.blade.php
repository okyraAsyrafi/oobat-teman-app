    ...
    <div class="overflow-y-auto py-5 px-3 h-full bg-white">
        ...
        <ul class="space-y-2">
            ...
            <li>
                <a href="{{ route('dashboard') }}"
                    class="flex items-center p-2 text-base font-medium text-gray-900 rounded-lg hover:bg-gray-100 group {{ request()->routeIs('dashboard') ? 'bg-gray-100' : '' }}">
            ...
            @if (Route::has('patient.index'))
                <li>
                    <a href="{{ route('patient.index') }}"
                        class="flex items-center p-2 text-base font-medium text-gray-900 rounded-lg hover:bg-gray-100 group {{ request()->routeIs('patient.*') ? 'bg-gray-100' : '' }}">
                        <svg class="w-6 h-6 text-gray-500 group-hover:text-gray-900" fill="currentColor"
                            viewBox="0 0 20 20">
            ...
            @if (Route::has('medication.schedules.index'))
                <li>
                    <button type="button"
                        ...
                        data-collapse-toggle="dropdown-schedules">
                        <svg class="w-6 h-6 text-gray-500 group-hover:text-gray-900" fill="currentColor"
                            viewBox="0 0 20 20">
                        ...
                        <span class="flex-1 ml-3 text-left">Jadwal Obat</span>
                    ...
                    <ul id="dropdown-schedules" class="hidden py-2 space-y-2">
                        <li>
                            <a href="{{ Route::has('medication.schedules.index') ? route('medication.schedules.index') : '#' }}"
                                class="flex items-center p-2 pl-11 text-base font-medium text-gray-900 rounded-lg hover:bg-gray-100">
                                Semua Jadwal
                            </a>
                        </li>
                        <li>
                            <a href="{{ Route::has('schedules.create') ? route('schedules.create') : '#' }}"
                                class="flex items-center p-2 pl-11 text-base font-medium text-gray-900 rounded-lg hover:bg-gray-100">
                                Tambah Jadwal
                            </a>
                        </li>
            ...
            @if (Route::has('educations.index'))
                <li>
                    <a href="{{ route('educations.index') }}"
                        class="flex items-center p-2 text-base font-medium text-gray-900 rounded-lg hover:bg-gray-100 group {{ request()->routeIs('educations.*') ? 'bg-gray-100' : '' }}">
                        <svg class="w-6 h-6 text-gray-500 group-hover:text-gray-900" fill="currentColor"
            ...
            @if (Route::has('questionnaire.index') || Route::has('question_answer.index'))
                <li>
                    ...
                    <ul id="dropdown-questionnaire" class="hidden py-2 space-y-2">
                        <li>
            ...
            @if (Route::has('telenursing.index'))
                <li>
                    <a href="{{ route('telenursing.index') }}"
                        class="flex items-center p-2 text-base font-medium text-gray-900 rounded-lg hover:bg-gray-100 group {{ request()->routeIs('telenursing.*') ? 'bg-gray-100' : '' }}">
                        <svg class="w-6 h-6 text-gray-500 group-hover:text-gray-900" fill="currentColor"
                            xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                            viewBox="0 0 24 24">
                            <path fill-rule="evenodd"
        ...
        @role('superadmin')
            <div class="mt-8 pt-8 border-t border-gray-300">
                <h3 class="px-3 mb-3 text-xs font-bold text-gray-500 uppercase tracking-wider">
                    Master Data
                </h3>
                <ul class="space-y-2">
                    ...
                    <li>
                        <a href="{{ Route::has('admin.users.index') ? route('admin.users.index') : '#' }}"
                            class="flex items-center p-2 text-base font-medium text-gray-900 rounded-lg hover:bg-gray-100 group {{ request()->routeIs('admin.users.*') ? 'bg-gray-100' : '' }}">
                            <svg class="w-6 h-6 text-gray-500 group-hover:text-gray-900" fill="currentColor"
                                viewBox="0 0 20 20">
                                <path
                    ...
                    <li>
                        <a href="{{ Route::has('admin.roles.index') ? route('admin.roles.index') : '#' }}"
                            class="flex items-center p-2 text-base font-medium text-gray-900 rounded-lg hover:bg-gray-100 group {{ request()->routeIs('admin.roles.*') ? 'bg-gray-100' : '' }}">
                            <svg class="w-6 h-6 text-gray-500 group-hover:text-gray-900" fill="currentColor"
                                xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                                viewBox="0 0 24 24">
                                <path
                    ...
                    <!-- Tambah master lain nanti di sini -->
                    <!-- Contoh: Data Obat, Data Puskesmas, dll -->
        ...
        <div class="absolute bottom-0 left-0 w-full p-4 bg-white border-t border-gray-200 z-20">
            <div class="flex items-center justify-between text-sm">
                ...
                <a href="https://obatteman.id" class="text-indigo-600 hover:underline font-medium">OBAT TEMAN</a>
            </div>
        </div>
</aside>
