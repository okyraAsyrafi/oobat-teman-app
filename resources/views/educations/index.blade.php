<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Kelola Edukasi</h2>
    </x-slot>
            ...
            <div class="bg-white shadow-md sm:rounded-lg overflow-hidden">
                ...
                <div
                    ...
                    <div class="w-full md:w-1/2">
                        <h3 class="text-lg font-semibold text-gray-900">Daftar Artikel Edukasi</h3>
                        <p class="mt-1 text-sm text-gray-600">
                            Artikel ini akan ditampilkan di aplikasi mobile untuk edukasi pasien.
                        </p>
                    </div>
                        ...
                        <tbody>
                            @forelse ($educations as $edu)
                                <tr class="border-b hover:bg-gray-50">
                                    <td class="px-4 py-3">
                                        @if ($edu->image_path)
                                            <img src="{{ Storage::url($edu->image_path) }}" alt="Gambar"
                                                class="h-10 w-10 object-cover rounded">
                                        @else
                                            –
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 font-medium text-gray-900">{{ Str::limit($edu->title, 40) }}
                                    </td>
                                    <td class="px-4 py-3">{{ number_format($edu->view_count) }}</td>
                                    <td class="px-4 py-3">
                                        @if ($edu->is_active)
                                            <span
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <div class="flex items-center justify-end space-x-2">
                                            <a href="{{ route('educations.edit', $edu) }}" title="Edit artikel"
                                            ...
                                            <form action="{{ route('educations.destroy', $edu) }}" method="POST"
                                                ...
                                                <button type="submit" title="Hapus artikel"
                                                    onclick="return confirm('Yakin hapus artikel {{ addslashes($edu->title) }}?')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-4 py-6 text-center text-gray-500">Tidak ada artikel.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                @if ($educations->hasPages())
                    <div class="p-4">{{ $educations->links() }}</div>
                @endif
