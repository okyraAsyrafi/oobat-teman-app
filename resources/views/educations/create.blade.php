<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Tambah Artikel Edukasi</h2>
    </x-slot>

    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 lg:px-6">
            <div class="bg-white rounded-lg shadow p-6">
                <form method="POST" action="{{ route('educations.store') }}" enctype="multipart/form-data" novalidate>
                    @csrf
                    ...
                    <!-- Upload Gambar -->
                    <div class="mb-4">
                        <label for="image" class="block mb-2 text-sm font-medium text-gray-900">Gambar Artikel
                            (Opsional)</label>
                    ...
                    <div class="mb-4">
                        <label for="content" class="block mb-2 text-sm font-medium text-gray-900">Konten
                            Artikel</label>
                        <textarea name="content" id="editor" rows="8">{{ old('content') }}</textarea>
                        @error('content')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
