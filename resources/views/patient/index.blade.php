<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Kelola Pasien</h2>
    </x-slot>
                ...
                <div
                    ...
                    <div class="w-full md:w-1/2">
                        ...
                        <p class="mt-1 text-sm text-gray-600">
                            Kelola data pasien TB. Pasien login menggunakan NIK dan tanggal lahir.
                        </p>
                    </div>
                        ...
                        <tbody>
                            @forelse ($patients as $patient)
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3">{{ $patient->date_of_birth->format('d-m-Y') }}</td>
                                    <td class="px-4 py-3">
                                        ...
                                        @else
                                            <span
                                                class="bg-red-100 text-red-800 text-xs px-2 py-1 rounded">Nonaktif</span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 text-right">
                                        <div class="flex items-center justify-end space-x-2">
                                            ...
                                            <form action="{{ route('patient.destroy', $patient) }}" method="POST"
                                                ...
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" title="Hapus pasien {{ $patient->name }}"
                                                    onclick="return confirm('Yakin hapus pasien {{ addslashes($patient->name) }}?')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-4 py-6 text-center text-gray-500">Tidak ada pasien.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
